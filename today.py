...
USER_NAME = os.environ['USER_NAME'] # 'SectionTN'
...
def recursive_loc(owner, repo_name, data, cache_comment, addition_total=0, deletion_total=0, my_commits=0, cursor=None):
    ...
    # cannot use simple_request(), because the file must be saved before raising Exception
    # GitHub's GraphQL API intermittently returns 502s on heavy pagination, so retry with backoff
        ...
        if repo_hash == hashlib.sha256(edges[index]['node']['nameWithOwner'].encode('utf-8')).hexdigest():
            try:
                if int(commit_count) != edges[index]['node']['defaultBranchRef']['target']['history']['totalCount']:
                    ...
                    print('   crawling', edges[index]['node']['nameWithOwner'], flush=True)
...
if __name__ == '__main__':
    ...
    age_data, age_time = perf_counter(daily_readme, datetime.datetime(2005, 8, 26))
